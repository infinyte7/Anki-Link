...
public class AnkiDroidHelper {
    ...
    public String getDecks() {
        ...
        if (deckList != null) {
            for (Map.Entry<Long, String> entry : deckList.entrySet()) {
                deckNames.add(entry.getValue());
            }
        }
        if (deckNames.isEmpty()) {
            return "";
        }
        JSONArray jsonArray = new JSONArray(deckNames);
        return jsonArray.toString();
    }

    public String getModels() {
        Map<Long, String> deckList = mApi.getDeckList();
        List<String> deckModels = new ArrayList<>();
        if (deckList != null) {
            for (Map.Entry<Long, String> entry : deckList.entrySet()) {
                deckModels.add(String.valueOf(entry.getKey()));
            }
        }
        if (deckModels.isEmpty()) {
            return "";
        }
        JSONArray jsonArray = new JSONArray(deckModels);
        return jsonArray.toString();
    }
}